...
-- where total price is (num_ordered * quoted_price) where only 1 num ordered and the order number is 21617
...
select order_num, quoted_price 
from order_line 
where quoted_price>500 and quoted_price<1000;
-- 14.	Create a new table of customer name, last name, and first name 
-- from customer and sales rep table by matching up their primary key
select c.customer_name, sr.last_name,sr.first_name
from sales_rep sr 
